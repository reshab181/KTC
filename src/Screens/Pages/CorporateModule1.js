...
const CorporateModule1 = ({navigation}) => {
  ...
  const [previousCity, setPreviousCity] = useState(''); // Track previous city selection
  ...
  // Add useEffect to handle city change and reset dependent fields
  useEffect(() => {
    const resetDependentFields = async () => {
      if (
        city_of_usage &&
        city_of_usage !== previousCity &&
        previousCity !== ''
      ) {
        console.log('City changed from', previousCity, 'to', city_of_usage);
        console.log('Resetting dependent fields...');

        // Reset Redux state for dependent fields - PROPERLY
        dispatch(
          updateCorporateSlice({
        ...
        // Alternative approach - dispatch individual updates
        dispatch(
          updateCorporateSlice({
            type: 'assignment',
            selectedItem: '',
        ...
        // Show alert to user about the reset
        Alert.alert(
          'Fields Reset',
          'City has been changed. Please re-select Rental Type, Car Group, and Pickup Address.',
          [{text: 'OK'}],
        );
      }

      // Update previous city tracker
      if (city_of_usage) {
        setPreviousCity(city_of_usage);
      }
    };

    resetDependentFields();
  }, [city_of_usage, dispatch, previousCity]);
  ...
  const handleBookingConfirmed = async () => {
    ...
    setTimeout(() => {
      dispatch(resetCorporateSlice());
      clearLocalFormState();
      setPreviousCity(''); // Reset previous city tracker
          ...
          <View style={styles.root}>
                  ...
                  {renderCustomTile(
                    ...
                    () => {
                      ...
                      if (!assignment) {
                        Alert.alert(
                          'Selection Required',
                          'Please select a Rental Type first.',
                        );
                        return;
                      }
            ...
            <Section title="Car Reporting Details">
              <View style={{backgroundColor: '#FFFFFF'}}>
                <CustomCalender rentalTypeValue={assignment} />
              </View>
              <View style={[styles.container2]}>
